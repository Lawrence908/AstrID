# AstrID Final Report Notes: Pipeline Analysis and Next Steps
...
## 7. Full-Catalog Sample Maximization (March 2026)

**Goal**: Address overfitting observed in training (see [TRAINING.md](../TRAINING.md)) by maximizing the number of training samples while keeping same-mission, same-filter pairs. Previous best-yield and curated sets yielded hundreds to low thousands of samples; the CNN showed strong val AUCPR on small curated sets but likely memorization. Scale target: order of magnitude more samples from the full catalog.

**References**: [DATA_PIPELINE.md](../DATA_PIPELINE.md) (stages, same-mission requirement, differencing), [TRAINING.md](../TRAINING.md) (workflow, run log, AUCPR, data sources).

### 7.1 Strategy: More Than One Pair per SN

The original differencing step produced **one** difference image per supernova (one preferred filter, e.g. SWIFT uuu or GALEX nd). Many SNe have multiple filters with both reference and science coverage (e.g. SWIFT uuu, uvv, ubb; PS1 g, r, i; HST f606w, f814w). Generating **all** same-mission, same-filter pairs per SN multiplies difference images—and thus triplets—without relaxing data quality.

- **Same-mission, same-filter retained**: No cross-mission or cross-filter pairing; differencing remains physically meaningful (matched PSF, pixel scale, wavelength).
- **Change**: `generate_difference_images.py` now supports `--all-filters`: it uses `find_all_matching_filter_pairs()` to emit every (ref, sci, filter, mission) tuple per SN instead of a single preferred pair. Existing diff files are skipped on re-run.
- **HST support**: HST was previously missing from `MISSION_FILTERS` in `src/domains/differencing/pipeline.py`; 148 same-mission HST pairs were being skipped. HST filters (e.g. f555w, f606w, f814w) and filename patterns (`HST_*`, `HLA_*`) were added so HST pairs are differenced and contribute to triplets.

### 7.2 Full-Catalog Pipeline and Numbers

**Config**: `configs/full_catalog.yaml` — all MAST missions (TESS, GALEX, PS1, SWIFT, HST), min-year 2000, full catalog query.

**Query / filter (current state)**:
- **Query**: 3,903 SNe queried (from 2,966 catalog entries with year ≥ 2000); 1,273 viable (both ref and sci).
- **Same-mission filter**: **759 SNe** (19.4%) with at least one same-mission ref/sci pair. Breakdown: GALEX 402, PS1 194, HST 148, SWIFT 95.

**Pipeline sequence run** (see DATA_PIPELINE for stage definitions):
1. **Query** (resume): `run_pipeline_from_config.py --config configs/full_catalog.yaml --stage query --resume`
2. **Filter**: `run_pipeline_from_config.py --config configs/full_catalog.yaml --stage filter`
3. **Download**: `run_pipeline_from_config.py --config configs/full_catalog.yaml --stage download` (skips completed SNe via pipeline_progress.json)
4. **Organize**: Run `organize_training_pairs.py` directly with `--input-dir` / `--output-dir` and `--decompress` (no `--clean`) so existing fits_training is not wiped when processing subsets.
5. **Differencing**: `run_pipeline_from_config.py --config configs/full_catalog.yaml --stage differencing --all-filters --visualize --no-skip-completed` so every same-mission same-filter pair is generated; existing diff FITS are skipped per file.
6. **Triplets**: `run_pipeline_from_config.py --config configs/full_catalog.yaml --stage triplets --triplets-visualize` → `train_real.npz`, `train_bogus.npz` under `output/datasets/full_catalog/training_triplets`.
...
| Training workflow & runs | [TRAINING.md](../TRAINING.md) |
