...
- Prefect UI: http://localhost:9004
...
Endpoint:
- POST http://localhost:9001/training/datasets/collect
...
Confirm:
- GET http://localhost:9001/training/datasets → returns newly created dataset.
- GET http://localhost:9001/training/datasets/{dataset_id} → shows counts.
...
Steps:
1. Ensure `API_BASE = "http://localhost:9001"` (no `/v1`).
