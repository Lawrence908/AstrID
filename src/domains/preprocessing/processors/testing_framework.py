...
class ProcessingPipeline:
    ...
    def process_single_image(
        ...
        processed_img, quality_metrics = self.image_processor.enhance_astronomical_image(
        ...
        if reference_image is not None:
            diff_img, diff_metrics = self.differencing_processor.perform_image_differencing(
                processed_img, reference_image, 
            ...
            quality_metrics['max_source_significance'] = max([s['max_significance'] for s in sources]) if sources else 0.0
    ...
    def batch_process(
        ...
        for i, img in enumerate(images):
            ...
            print(f"Processed image {i+1}/{len(images)} - Quality: {result.quality_metrics.get('snr', 0):.2f}")
    ...
    def validate_quality(self, result: ProcessingResult) -> Dict[str, bool]:
        ...
        validation['snr_acceptable'] = result.quality_metrics.get('snr', 0) >= thresholds['min_snr']
        validation['contrast_acceptable'] = result.quality_metrics.get('contrast', 0) >= thresholds['min_contrast']
        validation['noise_acceptable'] = result.quality_metrics.get('std', 0) <= thresholds['max_noise']
        validation['processing_time_acceptable'] = result.processing_time <= 30.0  # 30 seconds max
    ...
    def generate_quality_report(self) -> pd.DataFrame:
        ...
        for result in self.results_history:
            ...
            row = {
                ...
                'anomaly_score': result.quality_metrics.get('combined_anomaly_score', 0),
...
class ProcessingBenchmark:
    ...
    def benchmark_processing_methods(
        ...
        for method in methods:
            ...
            avg_sources = np.mean([r.quality_metrics.get('num_sources', 0) for r in batch_results])
...
class PerformanceAnalyzer:
    ...
    def analyze_processing_performance(self, pipeline: ProcessingPipeline) -> Dict[str, Any]:
        """Analyze processing performance and provide recommendations."""
