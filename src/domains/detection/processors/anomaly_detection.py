...
class AnomalyDetector:
    ...
    def detect_anomalies_unet(self, image: np.ndarray, threshold: float = 0.5) -> Tuple[np.ndarray, float]:
    ...
    def comprehensive_anomaly_detection(self, image: np.ndarray) -> Dict[str, Union[float, np.ndarray]]:
        ...
        # Combined score
        scores = [v for k, v in results.items() if 'score' in k and isinstance(v, (int, float))]
        if scores:
...
class SyntheticAnomalyGenerator:
    ...
    def create_synthetic_anomaly_dataset(
        ...
        anomaly_types: List[str] = ['bright_spots', 'dark_spots', 'streaks']
        ...
        for i in range(num_anomalies):
            ...
            if anomaly_type == 'bright_spots':
                # Add bright spots
                y, x = np.random.randint(0, base_img.shape[0], 2)
                base_img[y-2:y+3, x-2:x+3] += np.random.uniform(50, 100)
                ...
                # Add streaks
                y1, x1 = np.random.randint(0, base_img.shape[0], 2)
                y2, x2 = np.random.randint(0, base_img.shape[0], 2)
                import cv2
                cv2.line(base_img, (x1, y1), (x2, y2), int(np.random.uniform(50, 100)), 2)
